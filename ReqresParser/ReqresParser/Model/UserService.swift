...
class UserService {
    ...
    func loadUsers(page: Int, _ completion: @escaping (Result<UsersResponse, UserServiceError>) -> Void) {
        ...
        performDataTask(url: url, responseType: UsersResponse.self, completion)
    }
    
    func loadUser(id: Int, _ completion: @escaping (Result<UserResponse, UserServiceError>) -> Void) {
        guard let urlComponents = URLComponents(url: baseUrl.appendingPathComponent("users/\(id)"), resolvingAgainstBaseURL: false) else {
            return completion(.failure(.urlCreation))
        }
        guard let url = urlComponents.url else {
            return completion(.failure(.urlCreation))
        }
        performDataTask(url: url, responseType: UserResponse.self, completion)
    }
    
    private func performDataTask<Response: Codable>(url: URL, responseType: Response.Type, _ completion: @escaping (Result<Response, UserServiceError>) -> Void) {
        var urlRequest = URLRequest(url: url)
        urlRequest.httpMethod = "GET"
        urlRequest.setValue("text/json", forHTTPHeaderField: "Content-Type")
        urlRequest.timeoutInterval = timeoutInterval
        let dataTask = URLSession.shared.dataTask(with: urlRequest) { data, response, error in
            let result: Result<Response, UserServiceError>
            defer {
